...
### 8. Multi-User Data Isolation (Multi-Tenant)

- **REQ-8.1 User-scoped tables:** Categories, accounts, transactions, monthly_opening_balances, import_jobs, and assets must have a `user_id` column (FK to `"user"`) scoping all data to the owning user.
- **REQ-8.2 Category seeding:** On user signup (via Better Auth hook), seed the user with a default set of categories including their `llm_description` values extracted from the existing import prompt.
- **REQ-8.3 Category soft delete:** Categories use soft delete (`deleted_at` timestamp). Deleted categories remain visible in historical transactions and filters but cannot be selected for new transactions.
- **REQ-8.4 Category revive flow:** If a user attempts to create a category with a name matching a soft-deleted one, show an info prompt offering to revive the original. The user must choose a different name if they decline revival.
- **REQ-8.5 Category name uniqueness:** `UNIQUE(user_id, name)` constraint on categories, covering both active and deleted rows — no duplicate names allowed per user regardless of deleted status.
- **REQ-8.6 `llm_description` column:** Categories have an `llm_description` text column containing the guide text for the LLM parser. Users can edit this when creating/modifying categories. The LLM prompt for CSV import fetches only the user's active categories with their descriptions.
- **REQ-8.7 CSV import per-user:** The import pipeline (format detection, LLM parsing, worker execution) must accept/honor the authenticated user's context and write transactions to their accounts and categories.
- **REQ-8.8 Account isolation:** Bank accounts are scoped by `user_id`. CSV format parsers/formatters are shared globally (bank-level logic), but account instances belong to a single user.
